...
<html>
<head>
	<title>Customer Id Wise Report</title>
</head>
<body>
	<center><h3>Customer Id Wise Report</h3>
	<form action="custidreport.php " method="post">
		Please Enter Id :<input type="number" name="id" value="id"><br><br>
...
// $pdf->Table("select order_master.customer_id,order_detail.watch_id,")
// //$pdf->Table("select user_id,user_name,contact,email,address,date from customer where user_id='$id1' and date between'$d1' and '$d2'",$prop);
...
$prop=array(
			'padding'=>3);
$pdf->Table("select user_name,order_master.orders_id,order_master.date,watch_name,price,quantity,total from customer,order_detail,order_master where user_id=customer_id and order_detail.orders_id=order_master.orders_id and order_master.date between '$d1' and '$d2' and user_id='$id1'",$prop);
//$pdf->Table('select watch_id,watch_name,price,brand,category from watch order by watch_id limit 0,10',$prop);
$pdf->Output();
ob_end_flush();
}
